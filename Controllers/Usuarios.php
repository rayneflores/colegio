            ...
            if (empty($usuario) || empty($nombre) || empty($rol)) {
                ...
                if ($id == "") {
                    if($clave != $confirmar){
                        ...
                        if ($data == "ok") {
                            if ($rol == 3) {
                                $data = $this->model->preDocente($usuario);
                                if ($data == "ok") {
                                    $msg = array('msg' => 'Usuario Docente registrado con éxito', 'icono' => 'success');
                                } else{
                                    $msg = array('msg' => 'Error al registrar el Docente', 'icono' => 'error');
                                }
                            } elseif ($rol == 4) {
                                $data = $this->model->preRepresentante($usuario);
                                if ($data == "ok") {
                                    $msg = array('msg' => 'Usuario Representante registrado con éxito', 'icono' => 'success');
                                } else{
                                    $msg = array('msg' => 'Error al registrar el Representante', 'icono' => 'error');
                                }
                            } else {
